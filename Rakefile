...
namespace :test do
  task :prepare do
    # mkdir_p "Tests/APSHTTPClient Tests.xcodeproj/xcshareddata/xcschemes"
    # cp Dir.glob('Tests/Schemes/*.xcscheme'), "Tests/APSHTTPClient Tests.xcodeproj/xcshareddata/xcschemes"
  end

  desc "Run the APSHTTPClient Tests for iOS"
  task :ios => :prepare do
...
#desc "Run the APSHTTPClient Tests for iOS & Mac OS X"
desc "Run the APSHTTPClient Tests for iOS"
task :test do
...
def run_tests(scheme, sdk)
  sh("xcodebuild -workspace APSHTTPClient.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' -configuration Release clean test | xcpretty -c ; exit ${PIPESTATUS[0]}") rescue nil
end
