...
@implementation AppCHTTPPostForm
...
-(void)buildStringPostData
{
    ...
    for(NSInteger i = 0, len = [allKeys count]; i < len; i++)
    {
        ...
        [self appendStringData:[NSString stringWithFormat:@"%@=%@%@",
                          [AppCHTTPHelper encodeURL:key],
                          [AppCHTTPHelper encodeURL: [[self requestFormDictionay] valueForKey:key]],
                          (last ?  @"" : @"&")
                          ]
         ];
...
{
    [self addFormFile:path fieldName:name contentType:[AppCHTTPHelper fileMIMEType:path]];
}
...
-(void)addFormData:(NSData*)data fileName:(NSString*)fileName fieldName:(NSString*)fieldName
{
    [self addFormData: data
             fileName: fileName
            fieldName: fieldName
          contentType: [AppCHTTPHelper contentTypeForImageData:data]
     ];
