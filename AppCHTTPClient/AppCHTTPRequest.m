...
#import "AppCHTTPClient.h"

@implementation AppCHTTPRequest
@synthesize url = _url;
...
-(void)initialize
{
    ...
    _response = [[AppCHTTPResponse alloc] init];
    [_response setReadyState: AppCHTTPResponseStateUnsent];
}
...
-(void)abort
{
    ...
    if(_operation != nil) {
        ...
        [self connection:_connection didFailWithError:
         [NSError errorWithDomain:@"AppCHTTPErrorDomain"
...
-(void)send
{
    ...
    if([self synchronous]) {
        if([self requestUsername] != nil && [self requestPassword] != nil && [_request valueForHTTPHeaderField:@"Authorization"] == nil) {
            NSString *authString = [AppCHTTPHelper base64encode:[[NSString stringWithFormat:@"%@:%@",[self requestUsername], [self requestPassword]] dataUsingEncoding:NSUTF8StringEncoding]];
        ...
        [_response setReadyState:AppCHTTPResponseStateDone];
        [_response setConnected:NO];
    } else {
        [_response setRequest:_request];
        [_response setReadyState:AppCHTTPResponseStateOpened];
        ...
        if([self theQueue]) {
            RELEASE_TO_NIL(_operation);
            _operation = [[AppCHTTPOperation alloc] initWithConnection: self];
...
{
    DeveloperLog(@"Code %li Redirecting from: %@ to: %@",(long)[(NSHTTPURLResponse*)response statusCode], [_request URL] ,[request URL]);
...
{
    DeveloperLog(@"%s", __PRETTY_FUNCTION__);
    [_response setReadyState:AppCHTTPResponseStateHeaders];
    [_response setConnected:YES];
    [_response setResponse: response];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if([_response readyState] != AppCHTTPResponseStateLoading) {
        [_response setReadyState:AppCHTTPResponseStateLoading];
        if([_delegate respondsToSelector:@selector(tiRequest:onReadyStateChage:)]) {
            [_delegate tiRequest:self onReadyStateChage:_response];
        }
    }
